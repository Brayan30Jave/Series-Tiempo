...
\hypersetup{
  ...
  pdfauthor={Brayan Hernandez Cardona - Adriana Janneth Avila - WILLIAM ROJAS CARABALI},
  hidelinks,
...
\NormalTok{datos }\OtherTok{\textless{}{-}}\NormalTok{ readxl}\SpecialCharTok{::}\FunctionTok{read\_excel}\NormalTok{(}\StringTok{"MUESTRA SERIE TIEMPO.xlsx"}\NormalTok{)}
\FunctionTok{sum}\NormalTok{(datos}\SpecialCharTok{$}\NormalTok{quantity)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{datos\_ts }\OtherTok{\textless{}{-}} \FunctionTok{ts}\NormalTok{(datos}\SpecialCharTok{$}\NormalTok{quantity, }\AttributeTok{frequency =} \DecValTok{12}\NormalTok{, }\AttributeTok{start =} \FunctionTok{c}\NormalTok{(}\DecValTok{2017}\NormalTok{, }\DecValTok{1}\NormalTok{), }\AttributeTok{end =} \FunctionTok{c}\NormalTok{(}\DecValTok{2020}\NormalTok{,}\DecValTok{12}\NormalTok{)) }

\FunctionTok{plot}\NormalTok{(datos\_ts) }
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{datos}\OtherTok{\textless{}{-}}\NormalTok{readxl}\SpecialCharTok{::}\FunctionTok{read\_excel}\NormalTok{(}\StringTok{"MUESTRA SERIE TIEMPO.xlsx"}\NormalTok{)}
\NormalTok{datos}\SpecialCharTok{$}\NormalTok{transaction\_date }\OtherTok{\textless{}{-}} \FunctionTok{as.Date}\NormalTok{(datos}\SpecialCharTok{$}\NormalTok{transaction\_date, }\AttributeTok{origin =} \StringTok{"1900{-}01{-}01"}\NormalTok{)}

\NormalTok{datos }\OtherTok{\textless{}{-}}\NormalTok{ datos[}\FunctionTok{order}\NormalTok{(datos}\SpecialCharTok{$}\NormalTok{transaction\_date), ]}

\CommentTok{\# Aggregate the quantity sold per day (or other metric as needed)}
\NormalTok{agg\_data }\OtherTok{\textless{}{-}} \FunctionTok{aggregate}\NormalTok{(quantity }\SpecialCharTok{\textasciitilde{}}\NormalTok{ transaction\_date, }\AttributeTok{data =}\NormalTok{ datos, sum)}

\CommentTok{\# Convert the data to a time series starting from the earliest date}
\NormalTok{start\_year }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{format}\NormalTok{(}\FunctionTok{min}\NormalTok{(agg\_data}\SpecialCharTok{$}\NormalTok{transaction\_date), }\StringTok{"\%Y"}\NormalTok{))}
\NormalTok{start\_day }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{format}\NormalTok{(}\FunctionTok{min}\NormalTok{(agg\_data}\SpecialCharTok{$}\NormalTok{transaction\_date), }\StringTok{"\%j"}\NormalTok{))}

\CommentTok{\# Create a time series object from the aggregated data}
\NormalTok{quantity.ts }\OtherTok{\textless{}{-}} \FunctionTok{ts}\NormalTok{(agg\_data}\SpecialCharTok{$}\NormalTok{quantity, }\AttributeTok{start =} \FunctionTok{c}\NormalTok{(start\_year, start\_day), }\AttributeTok{frequency =} \DecValTok{365}\NormalTok{)}

\CommentTok{\# Plot the original time series}
\FunctionTok{plot}\NormalTok{(quantity.ts, }\AttributeTok{main=}\StringTok{"Sales Quantity Time Series"}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"Quantity"}\NormalTok{, }\AttributeTok{xlab=}\StringTok{"Year"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#convertir la variable "quantity" en una serie de tiempo diaria"}

\NormalTok{data}\SpecialCharTok{$}\NormalTok{transaction\_date }\OtherTok{\textless{}{-}} \FunctionTok{as.Date}\NormalTok{(data}\SpecialCharTok{$}\NormalTok{transaction\_date, }\AttributeTok{format =} \StringTok{"\%Y{-}\%m{-}\%d"}\NormalTok{) }\CommentTok{\# Ajusta el formato si es necesario}

\CommentTok{\#agrupar los datos por dia }

\FunctionTok{library}\NormalTok{(dplyr)}
\FunctionTok{library}\NormalTok{(tidyr)}

\CommentTok{\# Crear una serie diaria completa}

\NormalTok{daily\_data }\OtherTok{\textless{}{-}}\NormalTok{ data }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(transaction\_date) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{summarise}\NormalTok{(}\AttributeTok{quantity =} \FunctionTok{sum}\NormalTok{(quantity, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{complete}\NormalTok{(}\AttributeTok{transaction\_date =} \FunctionTok{seq}\NormalTok{(}\FunctionTok{min}\NormalTok{(transaction\_date), }\FunctionTok{max}\NormalTok{(transaction\_date), }\AttributeTok{by =} \StringTok{"day"}\NormalTok{),}
           \AttributeTok{fill =} \FunctionTok{list}\NormalTok{(}\AttributeTok{quantity =} \DecValTok{0}\NormalTok{))}


\CommentTok{\# Crear la serie temporal }

\NormalTok{quantity\_ts }\OtherTok{\textless{}{-}} \FunctionTok{ts}\NormalTok{(daily\_data}\SpecialCharTok{$}\NormalTok{quantity, }\AttributeTok{start =} \FunctionTok{c}\NormalTok{(}\FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{format}\NormalTok{(}\FunctionTok{min}\NormalTok{(daily\_data}\SpecialCharTok{$}\NormalTok{transaction\_date), }\StringTok{"\%Y"}\NormalTok{)),}
                                                 \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{format}\NormalTok{(}\FunctionTok{min}\NormalTok{(daily\_data}\SpecialCharTok{$}\NormalTok{transaction\_date), }\StringTok{"\%j"}\NormalTok{))),}
                  \AttributeTok{frequency =} \DecValTok{365}\NormalTok{)}


\CommentTok{\#dividir los datos en entrenamiento y prueba }

\NormalTok{train\_size }\OtherTok{\textless{}{-}} \FunctionTok{floor}\NormalTok{(}\FloatTok{0.8} \SpecialCharTok{*} \FunctionTok{length}\NormalTok{(quantity\_ts))}
\NormalTok{train }\OtherTok{\textless{}{-}}\NormalTok{ quantity\_ts[}\DecValTok{1}\SpecialCharTok{:}\NormalTok{train\_size]}
\NormalTok{test }\OtherTok{\textless{}{-}}\NormalTok{ quantity\_ts[(train\_size }\SpecialCharTok{+} \DecValTok{1}\NormalTok{)}\SpecialCharTok{:}\FunctionTok{length}\NormalTok{(quantity\_ts)]}
